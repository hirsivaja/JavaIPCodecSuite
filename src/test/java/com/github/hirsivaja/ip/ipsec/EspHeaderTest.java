package com.github.hirsivaja.ip.ipsec;

import com.github.hirsivaja.ip.IpPacket;
import com.github.hirsivaja.ip.IpUtils;
import com.github.hirsivaja.ip.TestUtils;
import com.github.hirsivaja.ip.ipv6.Ipv6Header;
import org.junit.Assert;
import org.junit.Test;

import java.nio.ByteBuffer;

public class EspHeaderTest {

    @Test
    public void espHeaderTest() {
        byte[] espBytes = IpUtils.parseHexBinary("");

        IpPacket packet = IpPacket.decode(ByteBuffer.wrap(espBytes));

        Ipv6Header header = (Ipv6Header) packet.header();
        EspHeader espHeader = (EspHeader) header.extensionHeaders().getFirst();
        Assert.assertEquals(0x3D713155, espHeader.spi());
        Assert.assertEquals(0x00000003, espHeader.seqNumber());
        Assert.assertEquals(144, espHeader.data().length());
        Assert.assertArrayEquals(espBytes, TestUtils.toBytes(packet));
    }
}
