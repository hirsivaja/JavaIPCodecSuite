...
public record StrictSourceRoute(byte pointer, List<Ipv4Address> routeData) implements IpOption {
    ...
    public IpOptionType optionType() {
        return IpOptionType.STRICT_SOURCE_ROUTE;
    }

    public static StrictSourceRoute decode(ByteBuffer in){
        byte pointer = in.get();
        List<Ipv4Address> routeData = new ArrayList<>();
        while(in.hasRemaining()) {
            routeData.add(Ipv4Address.decode(in));
        }
        return new StrictSourceRoute(pointer, routeData);
    }
}
